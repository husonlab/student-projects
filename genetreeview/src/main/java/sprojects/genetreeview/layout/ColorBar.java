/*
 *  ColorBar.java Copyright (C) 2024 Algorithms in Bioinformatics
 *
...
package sprojects.genetreeview.layout;
...
import sprojects.genetreeview.model.GeneTreeSet;
...
public class ColorBar extends HBox {
	...
	private final HashMap<Integer, sprojects.genetreeview.layout.ColorBarBox> id2colorBarBox;
	...
	public void initializeColorBar(GeneTreeSet geneTreeSet, Slider slider) {
		...
		for (int id : geneTreeSet.getTreeOrder()) {
			sprojects.genetreeview.layout.ColorBarBox colorBarBox = new sprojects.genetreeview.layout.ColorBarBox(geneTreeSet.getPhyloTree(id).getName(), id2color.get(id));
	...
	public void addColorBox(String treeName, int id, int position, Color color, HashMap<String, Object> furtherFeatures) {
		...
		sprojects.genetreeview.layout.ColorBarBox colorBarBox = new sprojects.genetreeview.layout.ColorBarBox(treeName, id2color.get(id));
	...
	public void removeColorBox(int id) {
		sprojects.genetreeview.layout.ColorBarBox boxToRemove = id2colorBarBox.get(id);
	...
	public void setNames(ObservableList<String> orderedGeneNames) {
		for (int i = 1; i < this.getChildren().size() - 1; i++) {
			...
			((sprojects.genetreeview.layout.ColorBarBox) this.getChildren().get(i)).setName(name);
		}
	}
